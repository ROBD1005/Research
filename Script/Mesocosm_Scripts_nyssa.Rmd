...
temp.data <- files %>%  # read csv files into one data frame
  set_names()%>%
  map_dfr(~ read_csv(.x, skip=4, col_names = c("Date", "Temp"), col_types = list(col_character(), col_double())), #skip first rows of metadata and rename columns
          .id="filename") %>%
  select(filename, Date, Temp)
  
  
  
  #labeling the File ID
